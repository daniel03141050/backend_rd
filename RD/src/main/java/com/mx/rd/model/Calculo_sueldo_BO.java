 ...
 * @author danie
 */
public class Calculo_sueldo_BO {
    private DataSource dataSource;
    private Request_calculo_pago obj_cp_response;
    private ArrayList<Calculo_sueldo_VO> jugadorArray;
    private Utilerias obj_utilerias;
    
    
    public String operacion(String obj_cp) 
    {
        ...
        obj2=obj_utilerias.check_json(obj_cp);
        if(obj2!=null)/*Testing error JSON*/
        {
            try
            {
                ...
                Type userListType = new TypeToken<ArrayList<Calculo_sueldo_VO>>(){}.getType();

                jugadorArray = gson.fromJson(S, userListType);
                
                if(jugadorArray != null)/*Algun jugador no existia*/
                {
                    /*
                        obtener el porcentaje de los equipos
                    */
                    Map<String, Long> equipos = jugadorArray.stream().collect(Collectors.groupingBy(Calculo_sueldo_VO::getEquipo, Collectors.counting()));
                ...
                {
                    System.out.println("---- Algun jugador no existe en BD");
                }
